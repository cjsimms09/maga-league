...
_Filed 2026-08-08. Computed by the certified Lab money-grader (`money_grade.py` +
`roster_sim.py`); the [certification gate](draft/tests/test_money_grade_certification.py)
reproduces all three seasons' actual money tables to the dollar, so this finding
is produced by the same machinery that is known-correct. Refresh: `python
draft/backtest/lab.py` (experiment **L0**)._
...
**$2,100 left on the table across 2023–25** (weekly-high + RS). That is real
money — the business case for the in-season lineup optimizer stated in dollars,
not points.
...
- **high-pool** = weekly-high leak (the dominant term; $330–420/team/yr).
- **entry/title** = playoff leak — *pending the bridge*, currently $0 here.
- **matchup** = regular-season record leak ($100–175/team/yr).

## The January grading hook

This is the **lineup optimizer's report card.** In January, grade the optimizer's
**realized** 2026 recovery against this baseline: of the ~$450–595/team the
