...
6. **Vote deadline chip** — still the only open half. **Chat reactions DONE (B built 09-02, relay recovered 09-05): 🔥 💀 🤡 on any locker-room message, one per person per emoji, your own shown as yours, 10/10 tests.** **Bet expiry chip DONE 09-02 (relay).** The vote deadline chip is still not buildable as specified: ballots carry no closing time (status open/closed_at only), so it needs a rule from A or Cory before it can be a chip rather than an invention.
7. ~~Pick'em vs the model~~ — **DONE (B built 09-02, relay recovered 09-05): the model's picks are frozen at lock and revealed with everyone else's, then graded on the same accuracy board — owners vs the machine, from the data we already had. The freeze is idempotent, so a second page load cannot recompute or overwrite what the model said. 10/10 tests.**
