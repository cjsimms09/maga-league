...
def main(argv: list | None = None) -> int:
    ...
    if promo and promo.get("blocked"):
        ...
        ledger.setdefault("blocked_promotions", []).append({
            ...
            # THE SHADOW RIDES THE BLOCKED PATH TOO (register 470 meets 199).
            # 470 attaches both rules' verdicts to every promotion so the loop
            # keeps measuring what each rule WOULD do. A blocked promotion is
            # exactly such a week — the MAE rule wanted to promote and the null
            # said no — so dropping the shadow here would blind 470 precisely
            # where 199 makes the interesting thing happen, and would contradict
            # this row's own principle that a block is a result, not a silence.
            "shadow": shadow,
        })
