...
def test_document_SURFACES_skipped_seasons_and_actual_source_by_season():
    """Register 4s: `regenerate()` bolts both fields onto its own return
    dict, but `document()` — the only thing that decides what is actually
    WRITTEN — used to drop them, so a season silently lost to a live-fetch
    failure left no trace in the committed artifact. MUTATION: leave either
    field out of `document()`'s return — the artifact goes back to looking
    complete when a season was dropped, which is the exact defect 4s is
    named for."""
    cal = {"cells": {}, "seasons": [2023, 2024], "min_n": 8, "graded": 5,
          "ungraded": 1, "cells_measured": 0, "cells_unmeasurable": 0,
          "caveat": "x", "band_note": "y",
          "skipped_seasons": [{"season": 2025, "reason": "nothing gradeable"}],
          "actual_source_by_season": {"2023": "committed_store", "2024": "live_fetch"}}
    doc = PE.document(cal)
    assert doc["skipped_seasons"] == [{"season": 2025, "reason": "nothing gradeable"}]
    assert doc["actual_source_by_season"] == {"2023": "committed_store", "2024": "live_fetch"}
...
# ── register 4s: prefer the committed store over a live re-fetch that can
# silently drop a season — 2025 was lost this way, twice, with no trace ────

def _write_store(path, season, weeks_points, scoring):
    """A minimal fixture store in the real on-disk shape
    `nflverse_weekly_store.save()` writes — built directly rather than via
    the writer, so this test does not depend on that module's own tests
    passing first."""
    import json as _json
    import nflverse_weekly_store as WS
    fp = WS.scoring_fingerprint(scoring)
    weeks = [{"season": season, "week": i + 1, "points": pts,
             "scoring": scoring, "scoring_fingerprint": fp,
             "row_count": len(pts)}
            for i, pts in enumerate(weeks_points)]
    path.write_text(_json.dumps({"weeks": weeks, "scoring_fingerprints": [fp]}))


def test_actual_from_committed_store_SUMS_ACROSS_WEEKS(tmp_path, monkeypatch):
    """MUTATION: return only the last week's points — a season's realized
    total is the SUM, not one week's snapshot; a player who scored in weeks
    1 and 2 must show both, not just the second."""
    scoring = {"pass_td": 6.0}
    store_path = tmp_path / "nflverse_weekly_points_2025.json"
    _write_store(store_path, 2025, [{"a": 10.0}, {"a": 5.0, "b": 3.0}], scoring)
    monkeypatch.setattr(PE, "__file__", str(tmp_path / "projection_error.py"))
    act = PE._actual_from_committed_store(2025, scoring)
    assert act == {"a": 15.0, "b": 3.0}


def test_actual_from_committed_store_REFUSES_A_SCORING_MISMATCH(tmp_path, monkeypatch):
    """⚠ THE WHOLE POINT — a store captured under a table that has since
    changed must never be silently reused. MUTATION: drop the fingerprint
    check — a stale store blends an old scoring rule into a calibration that
    claims to be measured under the current one, with nothing saying so."""
    store_path = tmp_path / "nflverse_weekly_points_2025.json"
    _write_store(store_path, 2025, [{"a": 10.0}], {"pass_td": 6.0})
    monkeypatch.setattr(PE, "__file__", str(tmp_path / "projection_error.py"))
    act = PE._actual_from_committed_store(2025, {"pass_td": 4.0})
    assert act is None


def test_actual_from_committed_store_ON_A_MISSING_FILE_IS_NONE(tmp_path, monkeypatch):
    monkeypatch.setattr(PE, "__file__", str(tmp_path / "projection_error.py"))
    assert PE._actual_from_committed_store(2099, {"pass_td": 6.0}) is None


def test_actual_from_committed_store_ON_THE_REAL_2025_FILE(monkeypatch):
    """Not a fixture — the actual committed store, the same discipline
    `test_load_player_weeks_READS_THE_REAL_COMMITTED_STORES` in
    test_nflverse_player_ceiling.py already applies. Confirms the real file's
    own scoring config round-trips through the fingerprint check."""
    import json
    doc = json.loads((Path(PE.__file__).resolve().parent
                      / "nflverse_weekly_points_2025.json").read_text())
    real_scoring = doc["weeks"][0]["scoring"]
    act = PE._actual_from_committed_store(2025, real_scoring)
    assert act and len(act) > 100, (
        "the real 2025 store should read back hundreds of graded players")


def test_regenerate_PREFERS_THE_COMMITTED_STORE_over_a_failing_live_fetch(monkeypatch):
    """The end-to-end register 4s fix: a live weekly-data fetch that raises
    for a season must not drop that season if a committed store covers it —
    reproduces the real 22:11 -> 00:37Z failure shape (2023/2024 fetch fine,
    2025 raises) with a fixture store standing in for the real committed one.
    MUTATION: remove the `_actual_from_committed_store` call from
    `regenerate()` (or call it after the live fetch instead of before) —
    every OTHER test in this file stays green while the next real dispatch
    loses a season to a transient network blip again, exactly as it did
    twice on 2026-08-17."""
    import pandas as pd
    import sleeper_import as SL
    import adp as ADP
    ...
    # `_actual_from_committed_store`'s OWN path-derivation and fingerprint
    # matching are covered by the unit tests above; this test's job is only
    # to prove `regenerate()` WIRES it in ahead of a failing live fetch, so
    # it is replaced wholesale rather than routed through `PE.__file__`
    # (which `regenerate()` also uses to locate `league_history.json` —
    # patching it would break an unrelated read in the same function).
    scoring = {"pass_td": 6.0}

    def fake_store(season, scoring_cfg):
        if season == 2025 and scoring_cfg == scoring:
            return {"1": 35.0}
        return None
    monkeypatch.setattr(PE, "_actual_from_committed_store", fake_store)
    ...
    # THE REAL FAILURE MODE, NOT A TOTAL ONE: A's diagnosis was that 2023 and
    # 2024 fetched fine while 2025 specifically raised — `regenerate()` has
    # an earlier `if not frames: return VOID` check that fires only when
    # EVERY year fails, which is a different (and separately real) gap this
    # fix does not claim to close. Simulating a total failure here would
    # exercise that earlier VOID path instead of the one under test.
    def _flaky_import(years):
        if 2025 in years:
            raise RuntimeError("simulated 404")
        return pd.DataFrame()
    monkeypatch.setattr("nfl_data_py.import_weekly_data", _flaky_import, raising=False)
    # No pbp recovery attempt for this test — it is a separate mechanism
    # (register 4s's own report notes it is real but did not fire in the
    # actual incident either) and a real call here would try live network.
    monkeypatch.setattr("nfl_data_py.import_pbp_data",
    ...
    monkeypatch.setattr(AsOfDataStore, "league_config", lambda self: {"scoring": scoring})
    monkeypatch.setattr(BB, "build",
                        lambda *a, **k: ({"season": 2023, "players":
                                         [{"player_id": "1", "position": "QB",
                                           "proj_mean": 100.0}]}, {}))
    monkeypatch.setattr(ADP, "build_adp_table", lambda *a, **k: {"adp": {}})

    cal = PE.regenerate()
    assert cal.get("actual_source_by_season", {}).get("2025") == "committed_store", cal
    cell = cal["cells"].get(("QB", "1-3"))
    assert cell is not None and cell["n"] >= 1, (
        "2025's committed-store total (35.0 for player 1) never reached a "
        "calibration cell: %r" % cal)
