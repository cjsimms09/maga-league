...
// ── basic render ─────────────────────────────────────────────────────────
{
  const html = V.render(mkRecs(), 'evidence sentence', null, "", esc);
  ...
  ck('a 4th+ recommendation is NOT rendered — three rows is enough per §2',
    (function () {
      const withFourth = V.render(mkRecs([{ player: { name: 'Extra Guy', adp: 9 }, position: 'TE', marginal: 10, why: 'bench only — he does not crack your lineup' }]), null, null, "", esc);
...
{
  const html = V.render(mkRecs(), null, null, "", esc);
  ...
  ck('a missing ADP degrades to an empty cell, not "ADP null" or a crash',
    (function () {
      const noAdp = V.render([{ player: { name: 'No ADP Guy' }, position: 'RB', marginal: 10, why: 'bench only — he does not crack your lineup' }], null, null, "", esc);
...
// ── the WHY string that must never be filtered (§4) ────────────────────────
{
  const html = V.render([{ player: { name: 'Bench Guy', adp: 40 }, position: 'QB', marginal: 3.2,
    why: 'bench only — he does not crack your lineup' }], null, null, "", esc);
...
// on the next fix), not a single caveat string. ───────────────────────────
{
  ...
  const withEvidence = V.render(mkRecs(), EV, null, "", esc);
  ...
  const withoutEvidence = V.render(mkRecs(), null, null, "", esc);
  ck('no evidence disclosure renders when nothing is given, rather than an empty <details>',
    !/rbm-evidence/.test(withoutEvidence));
  const partialEvidence = V.render(mkRecs(), { caveat: 'only this one' }, null, "", esc);
...
// ── honest empty states ────────────────────────────────────────────────────
{
  ck('no recs → empty string, not a broken shell', V.render([], { caveat: 'x' }, null, "", esc) === '');
  ck('null recs → empty string', V.render(null, { caveat: 'x' }, null, "", esc) === '');
  ck('undefined recs → empty string', V.render(undefined, { caveat: 'x' }, null, "", esc) === '');
}

// ── the board-agreement line — ported from A's independent build of this
// same panel (2026-08-20 merge: both of us built it, A's self-mounting
// duplicate was removed, this one genuinely-useful piece was kept). "does
// this panel's #1 agree with the board's own #1" is the reason it earns
// its screen space, so it must be stated, not left to the eye. ───────────
{
  const agree = V.render(mkRecs(), null, { topName: 'Jahmyr Gibbs', agrees: true }, "", esc);
  ck('agreement renders, names who both models want', /rbm-agree-yes/.test(agree) && /Jahmyr Gibbs/.test(agree));
  ck('agreement reads as a positive, not a warning', /agrees with the board/.test(agree));
  const disagree = V.render(mkRecs(), null, { topName: 'Someone Else', agrees: false }, "", esc);
  ck('disagreement renders distinctly, names the board\'s pick', /rbm-agree-no/.test(disagree) && /Someone Else/.test(disagree));
  ck('disagreement explains why it usually happens (a bench-only body at the top)',
    /would not start for you/.test(disagree));
  const noBoard = V.render(mkRecs(), null, null, "", esc);
  ck('no board comparison given → no agreement line at all, not a blank one',
    !/rbm-agree/.test(noBoard));
  const noName = V.render(mkRecs(), null, {}, "", esc);
  ck('a board object with no topName degrades the same as null — never "agrees with the board undefined"',
    !/rbm-agree/.test(noName));
}

// ── the explainPanel() passthrough — app.js's site-wide ⓘ-button pattern,
// PANEL_GUIDE['roster_builder']. Kept alongside (not instead of) the
// evidence <details>: this answers HOW TO READ the panel (the FLEX
// question Cory actually asked for), the evidence answers WHY TO TRUST it. ─
{
  const withExplain = V.render(mkRecs(), null, null, '<button data-explain-toggle="roster_builder">i</button>', esc);
  ck('the explain button renders inside the panel header, verbatim from app.js',
    /rbm-head[\s\S]*data-explain-toggle="roster_builder"/.test(withExplain));
  const noExplain = V.render(mkRecs(), null, null, '', esc);
  ck('an empty explain string renders no stray markup', !/data-explain-toggle/.test(noExplain));
  const missingExplain = V.render(mkRecs(), null, null, undefined, esc);
  ck('a missing (undefined) explain arg degrades the same as empty string, not "undefined" in the HTML',
    !/undefined/.test(missingExplain));
}
...
// ── escaping ────────────────────────────────────────────────────────────
{
  const html = V.render([{ player: { name: '<script>alert(1)</script>', adp: 1 }, position: 'RB',
    marginal: 5, why: '<script>alert(2)</script>' }], null, null, "", esc);
...
// ── KNOWN-POSITIVE (rule 3e): the real board, through mlv.js, renders ──────
{
  ...
  if (fs.existsSync(BOARD)) {
    ...
    const html = V.render(recs, RosterBuilderMLV.EVIDENCE, null, "", esc);
