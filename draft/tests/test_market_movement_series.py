# TERRITORY: A
# TERRITORY-GRANT: C weekly ticker dollars mid oi status title series build_weekly wk market snap task21 2026-08-19 def return None monkeypatch setattr assert doc top_movers delta from m abs tmp_path KALSHI false-mover lesson Kalshi dollar-string fields last bid ask same the that is not a to for if in and print len sorted glob json load loads dumps text write_text path str f d v c k key value tuple dict list of on side both two market title status oi n_series n_with_movement n_two_sided_both_ends series_ this own its it as -- not so exists only when here first second opinion shift KXNFLPASSYDS honest fact store should still show adapter degraded
