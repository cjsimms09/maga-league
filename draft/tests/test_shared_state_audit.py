...
FACTS = {
    "seat": {
        ...
        "exempt": {
            ...
            # BUDGET RESTORED TO 10 (2026-08-10, second pass). I first met this
            # audit by RAISING the budget to 12 with two exemptions. That was the
            # wrong move and Cory called it: the budget exists to force the
            # duplication down, and raising it converts a guard into paperwork.
            # The four league-seat reads now go through ONE accessor, leagueSeat(),
            # mirroring what mySlot() does for the room seat — including two that
            # were derived inside a SINGLE expression. The two identities stay
            # separate (kept_players.team_slot is stamped in LEAGUE seats while a
            # mock seats you elsewhere, and conflating them attributes my keepers
            # to a stranger's chair) but each now has exactly one reader.
            "app.js:leagueSeat": "the ONE derivation of the LEAGUE seat, paired with mySlot() "
                                 "for the ROOM seat",
        },
        "max": 10,
    },
