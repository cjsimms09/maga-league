...
// after somebody noticed. The ratchet is the guard that notices.
{
   ...
   *
   * RE-AIMED 2026-08-18, the day the archive landed: this block had kept a
   * PRIVATE ROUTES.md-only census, so when the tool's census went both-files
   * (an archived item is still closed — "I archived it" must not read as a
   * regression) the baseline moved and this copy went red on every archived
   * pair (C → A 68 -> 3 was the loudest). The test now measures the SAME
   * census the tool exports, so the two cannot drift apart again. */
  const live = R.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'ROUTES.md'), 'utf8'));
  const now = R.closureCensus(live);
