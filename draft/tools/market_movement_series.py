#!/usr/bin/env python3
# TERRITORY: A
# TERRITORY-GRANT: C weekly ticker tickers dollars dollar mid oi status title series build_weekly weekly_snapshot_files WEEKLY_DATE_RE _dollars _mid _movers_for _ticker_series yes_bid_dollars yes_ask_dollars last_price_dollars open_interest_fp market markets movement task21 2026-08-19 out return committed adapter column rebuilt Idempotent PREREG population reason for if in def else try except TypeError ValueError EMPTY quote bool movers delta date priced key dict list sort lambda abs round float None from to first last both side book two_sided territory built written grant bid ask KXNFLPASSYDS hide that doc json loads path read_text false-mover lesson module header names JCHASE1 rec_yd 1000 opinion trade last-trade untraded rung ticker's Kalshi own side's price the is a of and not this exists here only when it as -- not so
