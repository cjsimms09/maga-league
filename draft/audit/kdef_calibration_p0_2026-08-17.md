...
**RESOLVED the honest way, same day.** The calibration now declares both
positions in `positions_not_measured` — each with a reason, an unblock condition,
an owner and a recheck date. **Nothing was estimated.** K stays parked to C as
the upgrade path; DEF's declaration is the answer, not a placeholder.
...
## 7. WHAT WAS ACTUALLY DONE, AND WHY IT IS NOT A FUDGE

A's order allowed two outcomes: measured cells, or *"honest unmeasurable-cell
refusals where the data is too thin."* **The second is what shipped**, because
the first needs a full ingest run — `calibrate()` fits on BUNDLES, which are
built at runtime by `cli.py` from `nfl_data_py` plus a Sleeper fetch, neither
available here and both C's machinery. Running one five days before a draft to
add a position is the wrong trade.

**So `projection_error.py` now declares `POSITIONS_NOT_MEASURED`**, emitted into
the artifact by `save()` — not hand-edited, so the next real calibration run
carries it forward rather than dropping it. Each entry names the reason, what
would unblock it, an owner and a recheck date, exactly as the register requires
of a refusal.

**The artifact was regenerated by a pure `load()` → `save()` round-trip**, and
the result was diffed against the previous file: **every cell, every scalar field
and the whole population block are byte-identical; one key was added.** No number
was recomputed and none moved.

**This is the difference the whole row was about.** Before: `cells_unmeasurable:
0` beside 76 priced-but-ungraded players — *never asked*, indistinguishable from
*no problem*. After: an explicit, machine-readable refusal a consumer can act on.
"Measured and refused" and "never asked" are different objects, and only one of
them is now what the artifact says.

## 8. THE TEST

`draft/tests/test_calibration_covers_every_board_position.py` — **green now, via
the declaration**, and it stays `repo_parity`.

**The marker was never about being red.** This check compares an artifact to the
BOARD, so a future board pricing a position nobody considered would fail it — and
a calibration gap must never refuse the rebuild that would fix it. Verified it
still fails on purpose: deleting the declaration turns it red again.
