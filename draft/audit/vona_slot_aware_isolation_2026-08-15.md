...
WHAT CAME BACK (60 rooms per arm, seeds 1-60; late-% figures RESTATED
2026-08-15 after the metric fix below — rates, rounds, and shapes unchanged):

  METRIC CORRECTION, applied between this report's first draft and this
  version: picksLeftAtQB2 originally counted the QB2 pick itself among
  "picks left" (inclusive), while onesie_history_check.js — the source of
  the real-history bands every late-% here is compared against — counts
  exclusively (line 103, `pks.length - 1 - i`). The independent OpenAI
  review flagged the possible off-by-one; checking the historical tool
  proved it real. The simulator now counts exclusively, so sim and history
  are finally on one convention. Rates/rounds/shapes are unaffected; only
  the late-% classification moved.
    ...
    shipped: VONA_SLOT_AWARE=false (today's live default, both flags off)
      ...
      QB2 picks-remaining (cumulative): {"<=2":9,"<=3":25,"<=4":27,"<=5":27} of 32
      modal shape: QB2/RB6/WR4/TE1 (26.7%)

    off: VONA_SLOT_AWARE=true, VONA_WIRE_BENCH=false (vorp bench)
      RB=0 rooms: 0/60 (0%)
      QB2 rate: 100% (60/60), late when it happens: 10%
      QB2 picks-remaining (cumulative): {"<=2":0,"<=3":2,"<=4":2,"<=5":6} of 60 — early-to-mid draft
      modal shape: QB2/RB3/WR6/TE2 (70%)

    on: VONA_SLOT_AWARE=true, VONA_WIRE_BENCH=true (wire bench)
      RB=0 rooms: 0/60 (0%)
      QB2 rate: 100% (60/60), late when it happens: 100%
      QB2 picks-remaining (cumulative): {"<=2":3,"<=3":31,"<=4":54,"<=5":60} of 60 — endgame
      modal shape: QB2/RB6/WR4/TE1 (38.3%)

    real history (onesie_history_check.js): QB>=2 = 56.7%, 100% <=5 left.

  Determinism cross-check (run before the metric fix, on the then-current
  inclusive metric): the seeds-1-30 subset of the off/on arms reproduced
  the previous committed 30-room result EXACTLY — the new arm changed
  nothing about the old two. The metric fix shifts only how the same
  underlying picks are classified, not which picks happen: rates, rounds
  and shapes are seed-identical across both metric versions.
...
2. The wire comparison neither causes nor cures the RATE anomaly — both
   ...
   vorp-bench takes QB2 early-to-mid draft (90% NOT late under the
   history-matching metric), wire-bench pushes every single one into the
   <=5-remaining endgame band (100% late) — exactly where 100% of real
   historical QB2 picks land. The shipped default is also mostly late
   (84.4%). So the wire fix moves slot-aware QB2 timing to match history's
   band precisely — a real improvement WITHIN the slot-aware world — but
