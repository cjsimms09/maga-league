    ...
    for season in hist["seasons"]:
        ...
        for wk, entries in (season.get("weeks") or {}).items():
            for e in entries:
                ...
                ctl_rand.append(percentile(rng.choice(draws), draws))
                if orc is not None:
                    ctl_oracle.append(percentile(orc, draws))
    return rows, ctl_rand, ctl_oracle, skipped, skipped_started_unmapped, dropped_bench

def main():
    rows, ctl_rand, ctl_oracle, skipped, skip_started, dropped_bench = run()
    if not rows:
