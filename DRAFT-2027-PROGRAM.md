...
| 9 | **The seasonal multi-source blend is captured and PINNED before the sites flip.** CBS and ESPN stopped serving season-total projections on 2026-08-20 — 442/416 rows on 08-19, 0/0 on 08-20, same ffanalytics commit — two days before our draft, so the blend Cory drafted on was the 08-19 capture by luck, not by design (register 442). 2027: the last full seasonal capture is frozen by a dated pin no later than 08-15, the probe's `week` input is dispatched weekly in-season from week 1 so the weekly blend has twelve sources, and a source going dark pre-draft stays a red job (`zero_source_verdict`) | A (pin + gate) · C (weekly dispatch cadence) | `ffanalytics-probe.yml` nightly + `week=N` dispatches; `multisource_projections.py` pre/post-draft split | pin by 2027-08-15; weekly capture from 2026 week 1 | `test_multisource_zero_source_freeze.py` in CI; the seasonal artifact's `captured_at` stamp against the 2027 draft date |
