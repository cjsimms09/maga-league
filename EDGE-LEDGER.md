...
- **Why it's #1, now CONFIRMED by the participation test (2026-08-09):** the all-terms test
  found our whole measured edge is **the mask + the VALUE ANCHOR** (every adjuster is decoration
  or a drag). **The dollar figure is NOT transcribed here on purpose — read it from
  `draft/backtest/exp_participation.json`, which is regenerated by every Lab run.**
  - **⚠️ THIS FIGURE IS BOARD-DEPENDENT AND DRIFTS — WHICH IS WHY IT IS NO LONGER WRITTEN
    DOWN HERE.** It was $362 when this line was written and nobody moved it for two re-runs
    (2026-08-12 audit). On 2026-08-13 it did worse than go stale: A read 266.81 at commit
    74876c4 and C read 329.0 at tip 5efd076 ninety minutes later, because the Lab ran twice
    in between. The test guarding this figure therefore went RED ON A CRON, and since
