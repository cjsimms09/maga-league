...
| **Waiver** | **core + agreement proven + met real data** | src/routes/waivers.js: claim/drop/worth/who-else, prices via C1 (byte-identical to draft engine), dollars via shared $110/$100, consensus-alongside-dollars. Ran on REAL 2025 data (waiver_live_check.js) — found the thin-pool-VORP bug (see note), live adapter (waiverInputsFromBundle) wired with canonical-artifact VORP. PENDING: live Sleeper egress (blocked from sandbox) + post-draft rosters, C4 ledger emit, route/UI (B) |

**REALITY-MEETING FINDINGS (Cory's Q1 — wire to real data before B):**
- **Sleeper egress is BLOCKED from the sandbox** (403, org policy — reported, not routed around). And the **2026 season is undrafted** (draft Aug 22), so no live 2026 rosters/FA pool exist anywhere yet. Ran the tools on the last REAL season (2025) from the harvest instead — the real field shapes.
- **Bug found on real data (a synthetic fixture never would):** 48 of 254 real players carry an UNDEFINED position (now excluded, not crashed); and recomputing VORP over the thin available pool inflates RB/WR VORP → the waiver would value a player differently than the draft (a C1 violation). FIX wired: the live adapter attaches CANONICAL artifact VORP, never recomputes. The consensus-alongside-dollars column (C3) is what surfaced it.
| **Lineup** | **exists; audit closed** | matchupValue derived ($110, DONE); dual-objective deviation measured over 450 real team-weeks (DONE — deviates 10.9%, worth ~$9/season; NOT a big lever as modelled, honest finding) — **⚠️ AND THOSE FIGURES DESCRIBE A CONFIGURATION THAT HAS NEVER SHIPPED**: the harness supplies a per-player `sd` that production's `rosterView` never builds, and stripping it takes the same 450 team-weeks to **0.0% / $0.00** (2026-08-12; re-measure when B's measured SD lands, do not inherit). PENDING: consensus-alongside-dollars in the live view (C3), confirm predictions graded (C4) |
