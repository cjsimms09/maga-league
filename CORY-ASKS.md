...
|---|---|---|
| A10 | **WHAT TIME DOES THE DRAFT ACTUALLY START? — the one thing on draft night that no file in this repo can answer, and I was wrong about it twice.** I have twice said it is *"recorded nowhere"*. **That is false and the truth is more useful: `draft_time` IS a real config key**, settable by the commissioner at `/admin` (`src/routes/admin.js:827`), and `src/dashboard.js:133` reads `cfg.draft_time` and falls back to the literal `6:00 PM`, with a date fallback of `<season>-08-22`. *(Written out rather than quoted as code: the real expression contains a double pipe, and a raw pipe splits a markdown table cell — it broke this very row on the first attempt, which is the same class as the escaped-pipe bug in `register_recheck_check.js`.)* **NO COMMITTED JSON IN THIS REPO SETS IT.** Whether the live world store has it cannot be checked from here — it lives in the runtime blob store, not in git. **So the league is being shown 6:00 PM on 08-22 unless you set it, and that is a HARDCODED DEFAULT doing load-bearing work with no ruling behind it — the exact shape as the keeper deadline, where fifteen internal files said 08-20 and the hardcoded default (08-21) turned out to be right (P71).** **WHY IT MATTERS BEYOND THE DASHBOARD:** the runbook's Saturday sequence, the 03:00 CDT rebuild window (register 5i) and the freeze timing all hang off when the draft actually begins, and 5i's whole question is how many hours of margin you have. | **Only Cory knows what time the draft starts.** Everything else here is derived: the site shows a hardcoded `6:00 PM` fallback, no committed config overrides it, and whether the live world store has a value cannot be read from this repo. **One line settles it — *"draft starts at HH:MM"* — and I will write it into `league_config.json` verbatim the way the keeper ruling was, so the next reader gets a ruling instead of a fallback.** **DEFAULT if nothing is said: the site keeps showing 6:00 PM on 08-22, which is what it shows today.** ⚠️ **This is the same shape as the keeper deadline, where fifteen internal files said 08-20 and the hardcoded default was right (P71)** — so the fallback being unconfirmed does not mean it is wrong, only that nobody has checked. It matters beyond the dashboard: the runbook's Saturday sequence, the 03:00 CDT rebuild window and the freeze timing (register 5i) all hang off when the draft actually begins. |
