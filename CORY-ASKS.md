...
| A13 | ⚖️ **ONE RULING, AND IT IS A REAL TRADE-OFF RATHER THAN A RUBBER STAMP: POINTS OR ROSTER SHAPE?** Two engine changes are measured, on the same bundle, in one CI run, against a control that is exact (0.00 drift). **`need: 1.0` — +68.6 points per seat-year, better in 20 of 30 seats, positive in 3 of 3 seasons. And it is the WORST of five arms on roster shape:** it takes RB from 4.77 to **5.83** against the league's measured top-3 mean of 4.44, and drops WR from 5.10 to 4.33 against 5.00. **`VONA_SLOT_AWARE` — +58.2 points per seat-year, better in 21 of 30 seats, 3 of 3 seasons, and the BEST of five on shape** (distance 1.18 vs shipped 1.83), the only arm that moves RB *down* and TE *up* toward the winners at the same time. **So the honest framing is: `need` buys points by drafting MORE running backs. Slot-aware buys slightly fewer points and a roster that looks like the ones that win this league.** Your complaint has been the roster; the graded number favours the other one. **⭐ UPDATED SAME DAY — CONDITION 3 IS NOW MEASURED AND SLOT-AWARE PASSES IT DECISIVELY, WHICH IS THE STRONGEST THING ON THIS ROW.** I had marked it unmeasurable because the seat replay builds a legal lineup every week by definition and so can never report a missing one. Measured directly on the live board instead, with real byes: **your shipped configuration cannot field a skill lineup in two of the coming weeks — week 7 (QB) and week 10 (TE).** (K/DEF weeks are excluded — a one-kicker roster is un-fieldable in its own kicker's bye by construction and you just stream one.) **An empty starting slot is a zero on the scoreboard, and the defect reproduces on two independently built boards.** ⚠️ **I OVERSTATED THIS FOUR HOURS AGO AND AM CORRECTING IT BEFORE YOU RULE.** I first measured it on the pre-blend board and wrote *"slot-aware is the ONLY arm of four at ZERO"*. The board rebuilt at 05:11 with the multi-source mean, and **on the published board NO arm is at zero** — shipped 2, slot-aware 1, `need=1.0` 2, Auto 1. **What survives is the condition as I preregistered it — slot-aware has FEWER un-fieldable weeks than shipped on BOTH boards (3→0 and 2→1) — and that is what condition 3 asked.** What does not survive is the stronger sentence I added around it. **And the swing itself is a warning: slot-aware's simulated roster went WR6/RB3/TE3 to WR3/RB10/TE2 on one rebuild, so this probe can compare arms on a single board but must not be used to RANK them.** Register 74. **AND SLOT-AWARE STILL HAS A CATCH I WILL NOT TALK MYSELF PAST.** Four preregistered conditions, and it now passes THREE ON MEASUREMENT (the board-collapse test that originally switched it off, at modal share 0.9% against a 78.9% failure; the seat replay; and fieldability). **It FAILED condition 4: QB+TE went 3.07 → 3.47.** That rise is entirely TE (QB actually fell), and TE is the one position the league's winners genuinely draft more of — so I think I mis-specified the condition. **I am not shipping on a rationalisation I formed after seeing which way it broke.** `draft/audit/roster_shape_and_slot_aware_2026-08-19.md` has every number. **MY REC: ship slot-aware, hold `need` at 0.** DEFAULT if you say nothing by 08-21 6pm: **nothing ships** and you draft the configuration you have now. | Both are engine changes three days before your draft, and the choice between points and roster shape is yours, not a measurement. |
