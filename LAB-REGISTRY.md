...
- **State: CORE BUILT (2026-08-08).** The grading currency + honesty budget are live and validated:
  ...
  - **First experiment live — L0 (measurement):** weekly-high + RS dollars left on the table by lineup decisions — **+$520/637.50/520 per team** (2023/24/25, corrected 2026-08-15 — see EFFICIENCY-LEAK.md). Proves roster→scores→dollars end to end.
...
| opp-profiles | opponent tendency profiles (the room edge) | **BUILT + RUN (local, no egress)** | Per-owner from 468 matched picks 2023-25: position share vs field, first-round-by-position, early lean, onesie timing. **7/10 QB-late** (incl Cory); QB-early: Sadbru/cashworth/ds7mmet; cashworth RB-heavy, ds7mmet elite-TE, B8T3S/Richard2121 WR-heavy, Jreis RB-early. Feeds the survival/room model per-seat + the war-room matchup read. opponent_profiles.py, test 5/5. | REACH/which-board rides exp36_picks (adp+owner) in CI; timing/lean are ADP-free. No install — intel. |
| B0-keeper | keeper-conditional B0 (follow-within-need) | **FIRED — the draft-day rule** | From Cory's seat, keepers locked (Chase/Henry/Walker), 200 dossier rooms, paired null: **b0_need beats b0_pure by +\$258/season, CI [206,309]**. Mechanism confirmed by avg_RB (pure ADP drafts 4.55 RB vs need's 3.48 — the 4th-RB over-draft). Rule: recommend best-ADP AMONG UNFILLED STARTER NEEDS, then best-ADP for bench. EXP-KEEPER-B0.md, test 3/3. | reaches the pick-34 screen: rule = best-ADP among STARTABLE-capacity positions (value-depth beats strict fill-first +\$51, CI[17,88]; defer QB/DEF, take flex depth); never over-draft filled positions. Wire with the resolver. |
| 43 | full-board pick audit (all 450 picks, every owner) | **BUILT — pure core 7/7, egress WIRED + fired** | Cory's message #1: grade EVERY real pick market-relative (residual vs the leaguewide realized-vs-ADP curve), no replay/counterfactual. Answers which picks beat market (round×pos), does reaching pay (reach buckets), who drafts well (per-owner, CI), are-Cory's-41-different, dead-zone cross-check. Guards: n≥8 floor, bootstrap CI, BH-FDR q=0.10, LOSO sign. EXP43-PREREG.md + exp43_fullboard.py + test_exp43.py. Reuses exp36 spine (now emits exp36_picks.json). | reliability/skill SURFACE; roster-conditional question deferred to the grid (exp 44); no install |
| 25d | dead zone across 3 eras (BBM IV/V/VI full-N) | **FIRED — real but NOT universal** | Full-field dead-zone in the current era: **2023 + 2025 reproduce** the RB R5–6 collapse (VI: 143→81/79/73); **2024 BREAKS it** (RB strong R5–6, R1 depressed by the elite-RB injury year). "WR holds" is era-dependent too (steady WR decline in V/VI). EXP-DEADZONE-ERA.md. Board: tempered the deviation.js dead-zone caveat from "the shape" to "robust-but-breakable tendency; don't override elite RB value." 64/64. | calibrates an informational marker DOWN; still no re-weighting |
...
| Sunday lineup execution | "start these nine" | **L0 measured the LEAK ($520–637.50/team/season, corrected 2026-08-15) — the capture is untested** |
