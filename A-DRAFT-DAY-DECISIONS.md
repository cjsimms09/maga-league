...
| ~~**C2**~~ | ✅ **ALREADY RULED AND SHIPPED — this row was stale.** `09f94f99` shipped `MEASURED_WEIGHTS.ceiling 0 → 0.45` as Cory's ruling; the live engine confirms 0.45 and `app.js:52` seeds the board from it. This page (and `CLAUDE.md`) went on saying it was *"held at zero through the draft"*. Corrected 08-18. | **🟠 SEE 5g, WHICH I FILED TOO HIGH AND CORRECTED THE SAME DAY.** The restore button is pinned to a **2026-08-10** baseline carrying `ceiling: 0` and `stack: 0.5`, so one tap does revert this ruling and the D10 stack ruling, disclosing only a date. **But I measured the blast radius: 0 of Cory's 12 seats change** — both halves, isolated and together. Real mechanism, no Saturday consequence; **housekeeping before the next ruling, not a fire.** (Measured on the keepers-only pre-draft roster; `stack` is roster-dependent, so a mid-draft roster is not covered by that zero.) | **5g** filed, recheck 08-20 |
