...
| ~~**C2**~~ | ✅ **ALREADY RULED AND SHIPPED — this row was stale.** `09f94f99` shipped `ceiling 0 → 0.45`; the live engine confirms it. This page and `CLAUDE.md` went on saying it was *"held at zero through the draft"*. Corrected 08-18. | **🟠 5g, which I filed too high and corrected the same day:** the restore button is pinned to an 08-10 baseline, so one tap reverts this ruling and the D10 stack ruling — **but the measured blast radius is 0 of 12 seats.** Housekeeping, not a fire. | **5g** 🟠, recheck 08-20 |
