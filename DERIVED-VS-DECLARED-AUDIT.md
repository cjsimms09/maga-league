...
| ✅ **market-quality cuts** | `deviation.js` | ~~0.2/0.5~~ DERIVED | DONE — `EFF_CUTS` are the TERTILES of the measured positive-efficiency distribution (weak 0.156, well 0.472), recomputed from the surface on a re-fire; `<=0` (BACKWARDS) kept as the real zero-correlation constant. | derive from exp36 dist ✅ |
