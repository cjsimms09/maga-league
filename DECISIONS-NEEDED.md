...
- **CONFIDENCE.** Moderate, and bounded by **N = 1 season**. own_v6 exists only for 2025 (needs two prior weekly-points stores; 2021/2022 do not exist) and Sleeper's other seasons are refused. *(Corrected 2026-08-18, A: the 2021/2022 stores were built 08-17 — `199103e4` — so the parenthetical reason is stale; the Sleeper leak gates are the binding limit and N=1 stands for the three-way. own_v6-vs-FP alone now has 2023-25.)* The Sleeper-vs-FantasyPros WR gap of 0.0041 is not distinguishable from nothing. The population is the **354-player three-way intersection**, which is clean for comparison and is not a board.
