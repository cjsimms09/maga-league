...
| 8 | **Is the loop closed** — does the grade move the weights? | a grade nobody feeds back is a diary, not a learning system. |
...
arrives weekly and reaches nothing is a cost with no return, and it looks exactly
like success.
...
**No store gets a scheduled capture job until steps 4-8 have answers**, even if
the answer is "not yet, here is the prereg and the date". A weekly job for data
nothing consumes is a commitment to pay forever for something we have not decided
to use.
