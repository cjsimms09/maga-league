...
* **⛔ STANDING RULING — NO PAID PROPS, NO ODDS API, EVER AGAIN. Cory, 2026-09-01, verbatim: *"We don't need odds api remember!! ... We get free props from bovada, ball don't lie, etc!! Why can this never get fixed."*** It kept coming back because his 08-26 yes-to-the-cron plus "one month" was filed as an OPEN decision with a date (register 421, the runbook's standing dates, A21), and the GO sweep surfaced it every day until someone re-asked him. **A ruling is written HERE, closed, or it is not a ruling.** Consequences executed 09-01: `weekly-props-fetch.yml` schedule REMOVED (dispatch-only; the paid key is never renewed and nothing waits on 09-14); the props arm (P354, plan ①) is built on FREE sources — **Underdog, measured 09-02: game-week lines for every needed market across all 16 games, keyless (`free_props_census_2026.json`)** — with Bovada as the Thursday complement (P299) — **the writer is live: `free-props-writer.yml` wrote week 1 on 09-02, 312 players, and Tuesday's grader reads it;** Ball Don't Lie was probed 08-18 with the one stored key (`bdl_key_matrix.json`: 401 on odds, `Route not found` on every player-props spelling) — **that probe is only as good as that key; if Cory has a different BDL key or plan, C re-probes from Actions with a known-positive control, and nobody says "BDL is not the path" again without it.** If a free source fails, the answer is his own: *"we will find more sources"* — never a purchase ask.

* **⛔ TWO STANDING RULINGS, 2026-09-02.** (1) Cory, 2026-09-02, verbatim: "Keep everything private form other owners.. trust wire" — the access rule stands: every tool stays commissioner-only, owners get results (history) never tools; do not re-ask (A24, `ACCESS-RULE.md`). (2) The kicker — Cory, 2026-09-02, verbatim: "model not drafting a K is once again indicative of it not understanding fantasy.. you can't start anyone else in a kicker spot so not having one is not smart.. but at same time, it's probably right to wait til dead last pick as replacement value is null" — THE KICKER, FINAL: a K is REQUIRED (nobody else can start in that slot) and belongs to the DEAD-LAST pick (a replacement level near zero — the wire supplies one). The value function earns this without a cap — an empty required slot at the end of the draft costs the wire body (`mlv_grammar_probe.js` arm `mlv_bench_complete`): all 10 seats take the K with their last pick, every roster complete, 1.0 violations/seat, 1,921 projected startable pts vs humans 1,880. The roster grammar counts a missing K as incomplete and an early K as a waste.

* **🏈 HOW THE MODEL GETS AHEAD OF CORY BY 2027 — `FUTURE-PROOF-2027.md`** (Cory, 09-02: *"make it understand football better... know more than I do about the intricacies"*). The measured answer: not more football rules — we tested age curves, year-2 leaps, opportunity surges and expert projections and they LOST — but a season of evidence he cannot keep, a 🏈 ledger of football claims graded on this scoring (zero effects kept as knowledge), his own overrides captured WITH THE WHY and graded, and a January synthesis that admits nothing to the 2027 board without a graded prior. Queue of twelve claims in §2; E owns it, D measures, relay chases.
...
* **📈 SHIPPED 09-02 (A) — four self-learning instruments, all report-only where they touch a rule:** ① the live props converter now prices anytime-TD lines the way the backtest did (register 467; `fetch_weekly_props.implied_points`, one converter for both paths); ② the Tuesday grader runs the START/SIT promotion rule as a SHADOW beside the live MAE rule and writes both verdicts to `grades_2026.json` every week, override or not (register 470; P358 predicts they disagree by week 5 — Cory rules plan-⑥ on that evidence); ③ the 2025 weekly backtest replicates on 2024 with the claims fixed before the fold was read (register 471: props beats the champion at 4 of 4 positions again, blend best on every grade) — **and a fold another lane's prereg holds blind is now REFUSED at the harness door** (`draft/data/blind_folds.json`, register 472; 2023 is P347's, do not read it); ④ the tool's Tuesday waiver claim is graded against what Cory ACTUALLY claimed, no button (register 466 ①), which found register 473. ⑤ (later the same day) **the weekly champion had priced ZERO of the 23 kept players** — Cory's Chase/Henry/Walker had no weekly projection in the loop and the member matchup odds refused his lineup every week (registers 476, the ninth and tenth consumers of the keeper split; `test_board_readers_see_keepers.py` now guards every live board reader); ⑥ **`draft/tools/props_second_opinion.py`** runs after every Thursday snapshot: the champion beside the props-implied number on Cory's LIVE roster, the lineup each would start, the named swaps, and the matchup gap under both opinions (`second_opinion_<season>_w<week>.json`); ⑦ the forward guarantee is judged against the week's REAL first kickoff (registers 475/477 — week 1 kicks off Wednesday night; the snapshot now runs Wednesday 20:00Z and Thursday's run freezes); ⑧ **CBS and ESPN did not break — they stopped serving SEASON totals on 08-20 and serve WEEKLY numbers (register 478: `ffanalytics-probe.yml` `week=N` dispatch returned CBS 434 / ESPN 414 / FleaFlicker 406 / NumberFire 588 weekly rows; the seasonal blend is the 08-19 capture, frozen for the season, `multisource_projections.py` exits clean post-draft)** — routed to D as a challenger arm and C as a Wednesday cadence, A defaults 09-08; ⑨ every draft-window alarm in `external-adp-capture.yml` (composition drift, draftable loss, drop depth, undecoded inside pick 150) is RECORDED after `league_config.draft.start_date` and escalates only before it — the GO sweep is green, and a red on the 09-03 nightly is a NEW defect. **Read before quoting a weight or a fold: `engine.js:826` and `blind_folds.json`.**
* **🔁 CLOSED 09-02 LATE (relay), each with a pointer, none a rule change:** the free props writer now stamps a MEASURED per-player weekly `sd` on every priced player (`props_spread.py`, P355's feed — the E[$] solver reads `p.sd` and nobody wrote it; wiring routed to A) · `draft/data/source_registry.json` is seeded and read by `go_status.py` (FUTURE-PROOF §6.2) · thirteen workflows lost commits to a bare `git push` race and now rebase-and-retry · `stale_row_sweep.py` reads the STATUS CELL through the guard's own parsing (register 469; baseline restated 274 open / 99 named by a later commit — the prose method said 493/185) · THIS-WEEK.md carries the props second opinion and the What-Stuck line · A's D1/D4/D9 rulings are executed (register 479 + P364, ROUTES → D, site list stays B's). GO sweep: all 17 captures green at 02:23Z.
