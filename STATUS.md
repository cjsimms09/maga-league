...
| 7. Full surface | ◐ **IN PROGRESS.** SOURCE ✅ live (reads the stage); NEAR-MISS still absent (needs Stage 4 thresholds → D13). **Class-tagged STACK LINE ✅ DONE (grind #4)** — `liveStackRoutes` + Zone-2 line + LEAN badge, class derived from the evidence table, `stack_routes.test.js` 17/17, wired into CI. **Movement line remains** — needs the `lastRecommendation` snapshot substrate (DID IT MOVE / ALMOST MOVED); WOULD MOVE is latency-blocked (PARKED #11) |
| 8. Mock #5+ | pending |
