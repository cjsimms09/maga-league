...
- **§C missing-feature verdicts:**
  1. **LRM countdown strip (2b.6):** **✅ BUILT** — `renderLRM()` → `#lrm-strip`, cost-of-acting-early framing ("QB safe until 94 — taking one now spends a skill pick 6 picks early"); tracks the **startable tier** (top-12 QB/TE, any K/DEF streamer) not the always-gone #1; R-LRM proves the deadline moves under a run. Screenshot sent to Cory. **Two review items:** (a) **no DEF in the board** — the artifact's player pool has RB/WR/TE/QB/K only, **no DST at all**, so the tool can't rank/recommend defenses (real data-scope gap, flag); (b) LRM **tier threshold** for QB/TE (top-12) reads TE "safe until 141", underselling the elite-TE cliff — Cory's call whether to tighten to a tier-based threshold (DECISIONS D5).
