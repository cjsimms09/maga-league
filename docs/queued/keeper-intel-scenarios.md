...
- **Mock/rehearsal boards use the PREDICTED slates** (clearly marked as predicted) until real designations land, then replace them as real designations land. **There is no keeper-watch process** — an earlier draft of this file called it "existing" two lines above saying the piece was NOT YET BUILT. Real designations reach the board through the nightly `draft-data` rebuild, which reads them from live Sleeper. Rehearsing against a *probable* board beats rehearsing against a fictional full pool (the fixture-keepers lesson, generalized to opponents).
