...
- **Bench-as-lottery-tickets: MECHANISM unchanged, DOSE corrected by measurement (2026-08-08).** The mechanism stands and is right for both replacement-level and weekly-high reasons: a bench floor is free on the waiver wire, so a late flier is priced on ceiling alone (`upsideBonus`'s lateness × endgame multipliers — untouched). **But the AGGRESSIVE-ceiling dose was a designed hypothesis, and the Lab inverted it.** Exp 2 §5's per-phase grid: endgame ceiling **0.5 is BETTER (+$19/season, CI [7.5, 33])** while **1.0 / 2.0 / 3.0 are all WORSE with CIs excluding zero**; exp 21 found the same inverted-U from a different control. So the measured truth is **moderate ceiling everywhere, not aggressive-late** — the endgame weight came down 1.4 → 0.5. Stated plainly because it was OUR hypothesis: we designed the aggressive endgame ramp on reasoning that sounded right, and the money says the double-ramp (a heavy weight on top of upsideBonus's own late amplification) burns dollars. The lottery survives; the throttle came off the wrong end.
