...
| Q14 | 🔴 **Should upside carry any PLAYER-SPECIFIC input at all?** Measured 08-17: **0 of 535 players share a `proj_mean` and differ on any dispersion field** — ceiling, floor, sd, weekly_sd and variance are all pure functions of `(position, proj_mean)`. So a rookie and a ten-year starter with equal projections have identical modelled upside. | **This is the structural version of Cory's own instinct** (*"common logic shouldve told us that everyone having the same ceiling makes no sense"*). Until something player-specific reaches these fields, the board cannot answer *"who has more upside than his projection suggests"* — which is most of what he means by edge. Candidate inputs already ON DISK: age, rookie/year-2 status, target share, `vegas_lines_2021_2026.json`. | **M** | **A** |
