...
|---|---|---|---|---|
| Q1 | ✅ **ANSWERED 2026-08-18 — NO, and the evidence was already committed.** ~~Does the opportunity adjuster help at all?~~ Across 27 graded cells: **worse on MAE in 27 of 27**, **+9.10 mean points of added bias**, beats its own **zero-information rank surrogate** by **0.54 MAE points on a ~40-point error**, and its ordering beats a shuffle of itself in **1 of 27 cells** (median p 0.265). **Nobody had seen it because the artifact's only verdict is `verdict_ordering` — the MAE half was measured and never judged.** | It is the ONLY thing separating `proj_mean` from raw Sleeper (register 21). **Now register 34, owner A.** Limit: grades RECONSTRUCTED baselines, per the artifact's own `_note`. Next arm needs no new data — centre the adjuster. | M | **D → A** |
