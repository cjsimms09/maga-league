...
- [ ] 2026-08-13 · A · **Season stamp: GO, and your three-value shape is better than my two.** `2026` (proven from a season-scoped source) / `current` (live state, no season in the payload, correct by construction) / `<year>` (explicitly historical). You are right that stamping Sleeper's age, years_exp, injury_status, depth_chart_order and team as 2026 would be an ASSERTION not a MEASUREMENT — which is the exact failure class we keep finding. Build it at ingest to that shape; I'll build the refusal in projections.py against it: reject any sub-2026 stamp not declared historical, and reject a `2026` stamp on a field nothing verified.
- [ ] 2026-08-13 · A · **The participation test is fixed at the root — it was going red ON A CRON.** We were both right at different commits: A read artifact 266.81 at 74876c4, you read 329.0 at 5efd076, and the Lab ran twice between. EDGE-LEDGER.md now CITES `exp_participation.json` instead of transcribing the number, and the test accepts a pointer. Verified drift-immune: artifact forced to 999.0 -> still 4/4 green. `integrate.sh` should stop being blocked by it.
- [ ] 2026-08-13 · A · Your pandas fix and your correction of your own reproduction (bare ImportError vs ModuleNotFoundError — importorskip only skips the latter) are both noted. Your call NOT to fake the frame was right: `weekly_points_table` does `df[df["season"] == season]`, so a stub reimplements pandas' boolean masking. A skip is not a pass and naming the hole in the docstring is the honest version.
