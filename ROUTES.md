...
- [ ] 2026-08-13 · C · ⏳ **`TOP_N = 400` in `draft/proj_series.py` — PERISHABLE, and I have been parking it in the wrong place.** Parked four times in `PARKED.md:10017`, which the ROUTES protocol replaced; that is on me, so here it is in the channel you actually read. **It is biting right now:** `proj_series.json` holds 9 snapshots and every one is truncated to exactly 400, against a 1,759-player board. Each day past Week 1 the deep board is lost permanently — a preseason projection is only observable before the season and a retroactive fetch leaks (exp33). It matters because the deep bands are where the projection is *most* wrong: at `proj_rank 33+` it runs ~2× high at QB (0.479) and TE (0.522) vs ~1.1–1.45 early, and grading production against actuals first becomes possible January 2027 — off this archive or not at all. **Cost, measured not guessed:** 5.2 KB/snapshot at 400, so at the `MAX_SNAPS = 400` ceiling 400→2.0 MB, 576→2.9 MB, 1759→9.0 MB. **576 is the number of players carrying `proj_mean > 0` today**; I would set 700 for headroom, +1.0 MB at a ceiling 400 snapshot-days away. `MAX_SNAPS` is a separate cap on snapshot COUNT and is fine. Not mine by any rule — no C prefix matches and it is outside `draft/backtest/`. **Say a number and I will make the edit as a CROSS-LANE FIX with a banner**, or take it yourself; either is fine, but the cost of another week is a year of deep board.
